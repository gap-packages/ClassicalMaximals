...
# the list <A>gens</A>. The generators in <A>gens</A> are made immutable
# matrices over <A>F</A> in the process, which, among other things, will ensure
# that the attribute <Ref Func="DefaultFieldOfMatrixGroup"/> for the matrix
# group constructed is <A>F</A>.
InstallGlobalFunction("MatrixGroup",
function(gens, F)
    gens := List(gens, g -> ImmutableMatrix(F, g));
    return Group(gens);
end);

# Return a matrix group over the field <A>F</A> generated by the matrices from
# the list <A>gens</A> and set its size to <A>size</A>. The generators in 
# <A>gens</A> are made immutable matrices over <A>F</A> in the process, which, 
# among other things, will ensure that the attribute 
# <Ref Func="DefaultFieldOfMatrixGroup"/> for the matrix group constructed is <A>F</A>.
